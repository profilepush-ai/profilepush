import type { TextItem, FeatureSet, ResumeSectionToLines, ResumeWorkExperience } from '../../types';
...
const hasJobTitle = (item: TextItem) =>
  JOB_TITLES.some((jt) => item.text.split(/\s/).some((word) => word === jt));
...
export const extractWorkExperience = (sections: ResumeSectionToLines) => {
  ...
  for (const subsectionLines of subsections) {
    ...
    const [company] = getTextWithHighestFeatureScore(
