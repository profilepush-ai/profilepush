...
Parse ALL rows — do not skip any. If a row has minimal info, still create a profile with whatever is available.`,
  ...
  {
    id: 'ai-match-rank',
    name: 'AI Match — Score Shortlist 1-10',
    description: 'Scores the ~60 closest posts from the last 30 days against a pasted description (a consultant hotlist for bench sales, a job for vendors), 1-10 with a one-line reason.',
    source: 'supabase-function',
    location: 'supabase/functions/ai-match/index.ts',
    handler: 'scoreCandidates',
    model: 'Gemini (gemini-2.x-flash lineage)',
    userPrompt: 'You are an expert US IT staffing recruiter. Score how well each candidate below fits the brief, from 1 to 10: 9-10 excellent (same role, core skills, experience in range, visa and location compatible); 7-8 strong; 5-6 partial; 3-4 weak; 1-2 poor or disqualifying. Judge on substance, not keyword overlap. Treat a missing field as unknown, not a mismatch. Score every candidate. For each, write the reason as one complete sentence of 8 to 15 words naming the specific facts behind the score (matching or missing skills, experience, visa or location constraint). Never a single word or bare label.',
    jsonOutput: true,
    temperature: 0.2,
    maxTokens: 8192,
