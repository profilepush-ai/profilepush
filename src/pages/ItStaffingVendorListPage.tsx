...
export default function ItStaffingVendorListPage() {
  ...
  async function performDownload() {
    const { data, error: invokeError } = await supabase.functions.invoke<{ vendors: ActiveListContact[] }>('active-list');
    ...
    <div className="min-h-screen bg-white">
      <SEO
        ...
        jsonLd={{
          ...
          '@type': 'Dataset',
          name: title,
          description: 'A daily-refreshed directory of IT staffing vendors actively posting new job requirements, compiled from live market activity.',
      ...
      <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
        ...
        <p className="mt-3 text-sm leading-6 text-gray-600">
          Below are the vendors our system has seen most recently posting new job requirements — pulled from live market activity, not a stale monthly export. Emails are masked in this free preview; the vendors below have open requirements <em>right now</em>, not a snapshot from weeks ago.
        </p>
        <p className="mt-3 text-sm leading-6 text-gray-600">
          We show the 100 most recently active vendors below at no cost. <a href="/signup" className="font-semibold text-blue-600 hover:underline">Create a free ProfilePush account</a> to download the complete list with unmasked emails, plus daily digest emails as new vendors go active.
        </p>
        ...
        <div className="mt-3">
          {error ? (
            ...
            <ActiveListTable
              tabs={[{ key: 'vendors', label: 'Vendors', rows }]}
              activeTab="vendors"
        ...
        <section className="mt-10">
          ...
          <div className="mt-4 space-y-5">
            <div>
              <h3 className="text-sm font-semibold text-gray-800">How often is this vendor list updated?</h3>
              <p className="mt-1 text-sm leading-6 text-gray-600">The underlying data refreshes continuously as our system observes new vendor activity, and this page's cache updates hourly so what you see stays current.</p>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-gray-800">What counts as "active"?</h3>
              <p className="mt-1 text-sm leading-6 text-gray-600">A vendor is active if they've posted a job requirement in the past week. The list is sorted by most recently active first, and this free preview shows the top 100.</p>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-gray-800">Is the full list free?</h3>
              <p className="mt-1 text-sm leading-6 text-gray-600">The 100 most recently active vendors are shown here for free, with emails masked. Signing in to ProfilePush unlocks the complete, unmasked list as a downloadable CSV, plus the same view for bench sales/consultant contacts on our <a href="/it-staffing-bench-sales-recruiters-list" className="font-semibold text-blue-600 hover:underline">bench sales recruiters list</a>.</p>
            </div>
          </div>
        </section>
      </div>
