...
- `claim_market_stats_email_send(p_email)` — atomically claims the once-per-UTC-day send slot for an email, and permanently blocks unsubscribed addresses. This is what prevents duplicate sends when the same poster appears on many rows.
...
`BACKFILL_BATCH_SIZE` (`wrangler.toml` `[vars]`, default 25) and the cron cadence (`wrangler.toml` `[triggers] crons`, default every 30 minutes) control how fast the existing backlog of poster emails drains. Both are safe to tune without touching SQL — the backfill RPC's anti-join against `market_stats_email_sends` is the cursor, so no candidate is ever skipped or re-sent across runs regardless of batch size or frequency. Watch Mailgun's bounce/complaint dashboard after enabling the cron; slow it down if either climbs.
