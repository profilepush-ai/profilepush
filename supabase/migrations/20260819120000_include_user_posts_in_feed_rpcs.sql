-- Merge user-submitted posts into the same global Jobs/Hotlist feed RPCs
-- that already serve scraped LinkedIn content, so both sources appear
-- side by side (distinguished client-side by post_source). A self-posted
-- listing has nothing to be AI-matched against, so the user-post branch of
-- each union skips the radar_match_* join entirely and just always
-- appears (subject to hidden_at/post_status like everything else).
--
-- The return signature changes (4 new trailing columns), so this must be
-- DROP FUNCTION + CREATE rather than CREATE OR REPLACE — and dropping a
-- function loses its grants, so each one is explicitly re-granted below,
-- exactly matching what it had before this migration. This matters most
-- for get_pulse_social_feed_page: it's called with the anon key by the
-- Cloudflare pulse-feed-cache worker in front of /jobs.
