-- The dashboard's summary cards (Predictions Made, Submissions Today, Replies
-- Received, Total Vendors) are built around the old Predict/Reveal-per-card
-- and actual-send action model. Per-card Predict is gone, and nothing
-- actually sends anymore (Gmail Sync isn't wired up) — "Submit"/"Request" now
-- means an email was generated (logged to pulse_ask_ai_previews), and
-- "Preview" means the post content was viewed (logged to pulse_lead_actions).
-- Add today's counts of those, split by Jobs vs Hotlist, for the 4 new cards.
...
begin
  ...
  select jsonb_build_object(
    ...
    'total_vendors_today', (
      select count(distinct coalesce(nullif(j.poster_email, ''), j.company_name))
      from public.pulse_ask_ai_requests r
      join public.social_jobs j on j.id = r.job_id
      where r.account_id = p_account_id
        and r.job_id is not null
        and r.status = 'completed'
        and r.created_at >= v_today_start
        and coalesce(nullif(j.poster_email, ''), j.company_name) is not null
    ),
    'job_previews_today', (
      select count(*) from public.pulse_lead_actions a
      join public.social_jobs j on j.id::text = a.lead_id
      where a.account_id = p_account_id
        and a.action_type = 'post_content_viewed'
        and a.created_at >= v_today_start
    ),
    'job_submits_today', (
      select count(*) from public.pulse_ask_ai_previews
      where account_id = p_account_id
        and job_id is not null
        and updated_at >= v_today_start
    ),
    'hotlist_previews_today', (
      select count(*) from public.pulse_lead_actions a
      join public.social_hotlist h on h.id::text = a.lead_id
      where a.account_id = p_account_id
        and a.action_type = 'post_content_viewed'
        and a.created_at >= v_today_start
    ),
    'hotlist_requests_today', (
      select count(*) from public.pulse_ask_ai_previews
      where account_id = p_account_id
        and hotlist_id is not null
        and updated_at >= v_today_start
    )
  ) into v_summary;
