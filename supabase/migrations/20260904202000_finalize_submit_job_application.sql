-- Restores exception-safety around the chat-thread/seed-message block now
-- that it's confirmed working (20260904200500/201000 were the debug pass
-- with the real error surfaced and fixed) — a future edge case in this
-- block must never block the application itself from being recorded.
...
BEGIN
  ...
  BEGIN
    ...
    v_new_app_body := 'New application: ' || v_candidate_label || ' submitted for ' || v_job_title || '.';
    ...
    UPDATE public.post_chat_threads
      SET owner_unread_count = owner_unread_count + 1, last_message_at = now(),
          last_message_preview = LEFT(v_new_app_body, 140)
      WHERE public.post_chat_threads.id = v_thread_id;
