alter table public.pulse_predict_logs
  add column if not exists user_id uuid references auth.users(id) on delete set null;

create index if not exists idx_pulse_predict_logs_user_id
  on public.pulse_predict_logs (user_id);
...
revoke all on function public.get_pulse_dashboard_stats(uuid) from public, anon;
grant execute on function public.get_pulse_dashboard_stats(uuid) to authenticated;
