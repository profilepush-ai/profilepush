-- Adds post_count: how many underlying social_jobs/social_hotlist records
-- (within the active window) each contact has. Both RPCs' subqueries already
-- fan out one source row into several via LATERAL joins (visa/skills for
-- recruiters; skills/locations for vendors), so COUNT(*) would overcount —
-- count DISTINCT source row ids instead.
--
-- CREATE OR REPLACE can't change a function's return columns, so drop first.
DROP FUNCTION IF EXISTS public.get_active_list_recruiter_contacts_24h(integer);
DROP FUNCTION IF EXISTS public.get_active_list_vendor_contacts_24h(integer);
