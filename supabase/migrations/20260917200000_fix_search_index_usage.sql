-- Let the FTS index do its job.
--
-- The tsquery was built in a CTE and joined in, so the search predicate read
-- `q.tsq is null or src.search_document @@ q.tsq`. Because that depends on a
-- joined value, the planner cannot match it against the GIN index: the same
-- filter costs 13ms written directly against social_jobs and 56ms through the
-- view, but inside the RPC it degenerated into a full scan of the window plus
-- ts_rank_cd on every row -- 8s for a 30-day search, right at the
-- authenticated role's 8s statement_timeout.
--
-- Built inline from the parameter instead, both branches fold at plan time:
-- with no query the predicate disappears, and with one it becomes a constant
-- tsquery the index can satisfy.
