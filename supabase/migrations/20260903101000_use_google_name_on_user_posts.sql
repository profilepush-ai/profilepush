-- Self-submitted posts showed the poster's email username (e.g.
-- "sandy.garvit35") instead of their name whenever account_members.
-- display_name was empty — which it is for most Google sign-ins, since
-- that field is never backfilled from the OAuth profile. Google's real
-- name (raw_user_meta_data->>'full_name', same claim already used for the
-- header avatar) is available and should be preferred over the email-local
-- fallback.
...
-- Backfill existing user-submitted posts whose name is currently just the
-- email-local-part fallback, using the poster's Google full_name if they
-- have one now.
UPDATE public.social_jobs sj
SET posted_by_name = au.raw_user_meta_data->>'full_name'
FROM auth.users au
WHERE sj.post_source = 'user_post'
  AND sj.created_by_user_id = au.id
  AND NULLIF(au.raw_user_meta_data->>'full_name', '') IS NOT NULL
  AND sj.posted_by_name = split_part(au.email, '@', 1);

UPDATE public.social_hotlist sh
SET bench_sales_recruiter_name = au.raw_user_meta_data->>'full_name'
FROM auth.users au
WHERE sh.post_source = 'user_post'
  AND sh.created_by_user_id = au.id
  AND NULLIF(au.raw_user_meta_data->>'full_name', '') IS NOT NULL
  AND sh.bench_sales_recruiter_name = split_part(au.email, '@', 1);
