-- Add live counts of the account's own user-submitted Job/Hotlist posts
-- (from the new /posts feature) to the Pulse dashboard summary, for two new
-- "Jobs Posted" / "Hotlists Posted" cards. Counts every non-hidden post
-- (open + closed) so the number matches what /posts shows, not just today's
-- activity like the other cards.
  ...
  select jsonb_build_object(
    ...
    'jobs_posted', (
      ...
      where created_by_account_id = p_account_id
        and post_source = 'user_post'
        and hidden_at is null
    ),
    'hotlists_posted', (
      select count(*) from public.social_hotlist
      where created_by_account_id = p_account_id
        and post_source = 'user_post'
        and hidden_at is null
    )
