-- The dashboard is meant to be about this business's own activity only, not the
-- shared platform-wide market. "New Jobs Today" was a market metric (platform-wide
-- social_jobs count) and gets dropped entirely. "Total Vendors" was also wrong: it
-- counted distinct posters among today's new platform jobs, which has nothing to do
-- with this account's activity. It should count the distinct vendors this account
-- actually submitted a candidate to today (derived from completed job submissions).
