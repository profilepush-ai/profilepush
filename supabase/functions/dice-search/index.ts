...
const APIFY_ACTOR = "shahidirfan~dice-job-scraper";
const MAX_JOBS = 25;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const dicePostedDate = DATE_MAP[posted_date as string] ?? "24h";

    const { data: searchRecord, error: searchErr } = await supabase
      .from("dice_job_searches")
      .insert({
        keyword: normalizedKeyword,
        location: normalizedLocation,
        posted_date: dicePostedDate,
        account_id: account_id ?? null,
        user_id: user_id ?? null,
        status: "running",
      })
    ...
    if (!runRes.ok) {
      ...
      await supabase.from("dice_job_searches").update({ status: "failed" }).eq("id", searchRecord.id);
    ...
    await supabase.from("dice_job_searches").update({ apify_run_id: run.id }).eq("id", searchRecord.id);

    if (run.status !== "SUCCEEDED") {
      await supabase.from("dice_job_searches").update({ status: run.status === "FAILED" ? "failed" : "timeout" }).eq("id", searchRecord.id);
    ...
    if (items.length > 0) {
      ...
      const { data: inserted, error: insertErr } = await supabase.from("dice_jobs").insert(rows).select("id");
      if (insertErr) {
        await supabase.from("dice_job_searches").update({ status: "failed" }).eq("id", searchRecord.id);
      ...
      if (inserted && inserted.length > 0) {
        const embeddingPayload = inserted.map((r: { id: string }) => ({ type: "job", id: r.id, table: "dice_jobs" }));
        const EMB_BATCH = 20;
    ...
    await supabase.from("dice_job_searches").update({
    ...
    try {
      await supabase.from("api_usage_log").insert({
        ...
        function_name: "dice-search", provider: "apify", model: APIFY_ACTOR,
    ...
    const { data: finalSearch } = await supabase.from("dice_job_searches").select("*").eq("id", searchRecord.id).single();
    const { data: savedJobs } = await supabase.from("dice_jobs").select("*").eq("search_id", searchRecord.id).order("created_at", { ascending: true });
